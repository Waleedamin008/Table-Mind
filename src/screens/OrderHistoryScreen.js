...
import { Logo, OnlineBadge, Badge } from '../components/Shared';
import { Search, Filter, Home, Clock, CheckCircle, AlertCircle, RefreshCw } from 'lucide-react';
...
export default function OrderHistoryScreen() {
  ...
  const allOrders = [
    ...pendingSyncOrders.map(o => ({
      id: o.id, table: o.table, type: o.type,
      items: o.items.map(i => `${i.name} x${i.qty}`),
      total: 0, payment: 'Pending', syncStatus: 'Pending Sync',
      serviceTime: null,
  ...
  const syncColor = (status) => {
    ...
    if (status === 'Pending Sync') return 'amber';
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid var(--border)', padding: '14px 24px', position: 'sticky', top: 0, zIndex: 10, display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search order or table..."
            style={{ padding: '7px 10px 7px 30px', borderRadius: 8, border: '1.5px solid var(--border)', fontSize: 12, fontFamily: 'var(--font-body)', background: 'var(--cream)', outline: 'none', width: 200 }} />
        </div>
        <OnlineBadge />
        ...
        <div style={{ background: '#fff', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
          ...
          {filtered.map((order, i) => (
            <div key={order.id} style={{
              ...
              <div style={{ fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.5 }}>{order.items.slice(0, 2).join(' · ')}{order.items.length > 2 && ` +${order.items.length - 2} more`}</div>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--espresso)', fontFamily: 'var(--font-display)' }}>
                {order.total ? `GH₵ ${order.total}` : '—'}
              </div>

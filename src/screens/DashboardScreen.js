...
import { Logo, OnlineBadge, Card, Badge, Button } from '../components/Shared';
import {
  LayoutDashboard, ShoppingBag, Package, Users, BarChart2, Settings, Bell,
  TrendingUp, TrendingDown, Clock, AlertCircle, ArrowUpRight, Home
} from 'lucide-react';
...
const NAV = [
  ...
  { id: 'reports', icon: <BarChart2 size={15} />, label: 'Reports' },
...
export default function DashboardScreen() {
  const { dashboardStats, kdsOrders, setCurrentScreen, isOnline } = useApp();
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', fontFamily: 'var(--font-body)' }}>
      ...
      <nav style={{
        ...
        <div style={{ marginBottom: 24, paddingLeft: 8 }}><Logo size="sm" /></div>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2 }}>
          {NAV.map(item => (
            <button key={item.id} onClick={() => setNavActive(item.id)} style={{
        ...
        <div style={{ paddingTop: 12, borderTop: '1px solid var(--border)' }}>
          ...
          <button onClick={() => setCurrentScreen('home')} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', width: '100%', border: 'none', background: 'transparent', cursor: 'pointer', color: 'var(--text-muted)', fontSize: 12, fontFamily: 'var(--font-body)' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', background: 'var(--cream)' }}>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>Here's what's happening in your restaurant today · May 12 – May 18, 2025</div>
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <button style={{ background: 'var(--terracotta)', color: '#fff', border: 'none', borderRadius: 8, padding: '7px 14px', cursor: 'pointer', fontSize: 12, fontWeight: 600, fontFamily: 'var(--font-body)' }}>
              All Outlets ▾
            </button>
          </div>
        </div>
        ...
        <div style={{ padding: '24px' }}>
            ...
            <Card style={{ padding: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
                {[
                  ...
                  <div key={s.label} style={{ flex: 1, background: 'var(--cream)', borderRadius: 10, padding: '12px', textAlign: 'center' }}>
          ...
          {/* Bottom CTA */}
          <div style={{ marginTop: 20, background: 'linear-gradient(135deg, var(--espresso), var(--espresso-light))', borderRadius: 'var(--radius-lg)', padding: '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
